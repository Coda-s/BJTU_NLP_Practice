...
class ESIM(nn.Module):
    def __init__(self, args):
        ...
        self.lstm1 = nn.LSTM(self.embedding_dim, self.hidden_dim, batch_first=True, bidirectional=True)
        self.lstm2 = nn.LSTM(self.hidden_dim * 8, self.hidden_dim, batch_first=True, bidirectional=True)

        self.fc = nn.Sequential(
            nn.Linear(self.hidden_dim * 8, self.target_size),
            nn.Softmax(dim=-1)
        )
    ...
    def forward(self, data1, data2):
        ...
        result1, _ = self.lstm2(F.relu(concat1))
        result2, _ = self.lstm2(F.relu(concat2))
